...
**The 20-run fraction gate vs the single-run MDD.** `MDD_99` above is the floor for a *single* run's `|t| ≥ τ` decision. The strict gate instead runs each op `N = 20` times and fails at **≥2 of 20** over threshold, tolerating ≤1 lone transient ([security.md](security.md#empirical-timing-verification), `nix/gate.sh`). These reconcile because a *real* leak of size `Δ` is *consistent*: at `Δ = MDD_99` the per-run power is ≥99 %, so it fires in ~all 20 runs and clears the ≥2/20 bar with near-certainty — the single-run `MDD_99` is therefore a slightly *conservative* bound on the gate's floor, and the 20-run design buys *robustness to transients*, not a lower floor (the fraction gate's 1-transient tolerance does not raise the floor, because a genuine leak is not a lone transient). Note the gate does **not** pool the 20 runs; pooling would give a √20 ≈ 4.5× lower floor and is an `n`-budget option to weigh in #79.
