...
**The 20-run fraction gate vs the single-run MDD.** `MDD_99` above is the 99 %-power floor for a *single* run's `|t| ≥ τ` decision. The strict gate instead runs each op `N = 20` times and fails at **≥2 of 20** over threshold. In principle that aggregation lowers the floor via the order statistic: *if* the 20 runs were independent with equal per-run detection probability `p`, the gate would detect a consistent leak with `1 − (1−p)²⁰ − 20p(1−p)¹⁹`, which exceeds 99 % at `p ≈ 0.29`, giving a floor `Δ ≈ 5.6·σ/√n` — about **1.7× below** the single-run `MDD_99 ≈ 9.66·σ/√n`. **That independence is not established, though**: the 20 runs are sequential on one machine and the harness replays a fixed input/class seed, so shared thermal / frequency / machine-state effects can positively correlate their t-statistics — which makes the binomial *overstate* the aggregate power. So the honest default is to **report the conservative single-run bound** (the *maximum* per-run `MDD_99` across the 20 runs, §8), which does not depend on the independence assumption. The ~1.7× order-statistic gain — and the √20 ≈ 4.5× from *full* pooling — become available only after run-independence is validated (or the correlation modelled), an analysis/`n`-budget item for #79. (The `≥2/20` tolerance does not weaken this bound: the gate catches a leak that fires in **≥2 of 20** runs — consistent *or* intermittent — and only *misses* one that fires in ≤1/20, i.e. at most ~5 % of runs, which is at the measurement noise floor and indistinguishable from a lone transient. It does not assume the leak is perfectly consistent, only that a real one fires in more than one run in twenty.)
