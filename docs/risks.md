...
**Partial constant-time verification.** The C extension's constant-time properties have been empirically tested using a dudect-based timing harness (Welch's t-test, |t| < 4.5 threshold). Field arithmetic operations (`fred`, `fsub`, `fneg`, `fadd`) pass — their branchless conditional selection produces no detectable timing variation across 1.5 million measurements per function. However, the Montgomery ladder (`scalar_multiply_ct_internal`) fails the timing test (|t| = 875 at 10,000 measurements). The root cause is identified: `jp_add_internal` contains early-return branches on `uint256_is_zero(&p[2])` (infinity checks). Inside the ladder, the accumulators start at infinity (Z=0), and how quickly they escape this state depends on the scalar's bit pattern — undoing the constant-time property that the branchless `cswap` provides. This is a documented, measured gap awaiting a fix (branchless `jp_add`). Field arithmetic claims are now empirically grounded; the scalar multiplication leakage is known and characterised rather than unknown.
