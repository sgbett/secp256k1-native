...
**Unverifiable constant-time claims in Ruby.** Even the C extension's constant-time properties have not been verified with tools like ct-verif or dudect. The claims are based on code inspection (branchless conditional selection, bitwise masking) rather than empirical measurement.
