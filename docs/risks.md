...
**Empirical constant-time verification.** The C extension's constant-time properties are empirically tested at two levels: a deterministic ctgrind/valgrind secret-poisoning check (data-flow, trustworthy on any host — run on the reference machine and locally via `security/run-checks.sh`; **not** in CI, which instead runs the companion **CT assembly-invariant** — the compiled ladder + `jp_add_internal` contain no secret-dependent branch/cmov — since ctgrind needs valgrind) and a statistical dudect timing harness (Welch's t-test, |t| < 4.5 threshold) run on quiet bare-metal hardware. Field arithmetic operations (`fred`, `fsub`, `fneg`, `fadd`) pass the security-relevant test — ctgrind finds no secret-dependent control flow, and the statistical timing (≈1.5 million measurements per function) shows no *secret-correlated* variation. This is distinct from "no detectable timing variation": some do carry a measurable **operand-value** microarchitectural artefact that is toolchain-dependent (on the GCC 15.1 reference machine `fsub` and `jp_add` reach mean |t| ≈ 8 and ≈ 24, well above the near-noise-floor GCC 15.2 figures — operand-magnitude latency, not a branch; see [security.md](security.md#empirical-timing-verification)). The Montgomery ladder (`scalar_multiply_ct_internal`) — the secret-scalar operation that actually matters — stays flat on both toolchains (mean |t| = 0.68 on GCC 15.2, 0.52 on the GCC 15.1 reference machine; 0/20 runs over 4.5). These harnesses recompile the extension sources standalone at vanilla `-O2` (hardening-off), so they exercise *shipping-shaped* — not byte-identical — codegen: the gem compiles per-user via `mkmf`, so no single canonical `.so` exists to match. Codegen *identity* of the constant-time property is asserted separately and deterministically (branchlessness of the ladder + `jp_add_internal` on the real source, `nix/vanilla-ext.sh`); the residuals — per-user CFLAGS unmeasured, objdump invariant scoped to two symbols — are documented in [security.md](security.md#empirical-timing-verification).

This claim has survived two empirically-found regressions, each a demonstration of Principle 2 (empirical over inspected):

1. An early version had a measured leakage (|t| = 875) caused by early-return branches in `jp_add_internal` on `uint256_is_zero(&p[2])` (infinity checks). Inside the ladder the accumulators start at infinity (Z=0), and how quickly they escaped that state depended on the scalar's bit pattern — undoing the constant-time property that the branchless `cswap` provided. The fix replaced branching `jp_add_internal` with a fully branchless implementation using mask-based `uint256_select` for all input-dependent special cases.

2. That `uint256_select` fix was itself silently broken by the **compiler**. Bare-metal dudect verification for v1 (issue #25; AMD Ryzen 9 9950X, GCC 15.2, `-O2`) measured |t| ≈ 21 in the ladder. GCC 15.2 had recognised the branchless `(a & ~mask) | (b & mask)` select idiom, reconstructed the secret boolean, and emitted a conditional jump — a leak invisible to source inspection and to the deterministic check on the *previous* toolchain. The fix routes every constant-time select mask through a value barrier (`ct_value_barrier_u64`) that keeps the mask opaque to the optimiser. This is also why the bare-metal dudect pass is now a required pre-tag release gate, not a one-off: a constant-time *source* is not a constant-time *binary*, and only a statistical run on quiet hardware observes the compiled timing.
