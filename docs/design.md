...
The C extension exists primarily to provide constant-time guarantees that Ruby's interpreter cannot offer. The performance improvement (see [performance](performance.md)) is a welcome consequence of the same fixed-width arithmetic design, not the driving motivation.
