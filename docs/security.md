...
- **`fred`/`fsub`-style field tests:** the dudect input classes are deliberately *operand-magnitude-asymmetric* (e.g. one class subtracts a tiny value from a full 256-bit one, the other swaps the roles) to stress the branchless correction path. The residual |t| reflects that synthetic magnitude asymmetry in the adder/borrow path, not a secret-dependent branch — ctgrind confirms no secret reaches a *branch*, but this is an operand-value *latency* effect ctgrind cannot observe, so its benignness with respect to the secret scalar rests instead on the flat **strict** `scalar_multiply_ct_internal` measured end-to-end on real operands (the borrow path is exercised there too, uncorrelated with scalar bits). Tiny operands *do* occur on the secret path: the Montgomery ladder initialises its accumulator to the point at infinity `[0,1,0]` and processes those tiny (0/1) coordinates through `jp_add`/`jp_double` — including their `fsub` calls — for a *scalar-dependent* number of leading iterations (≈255 − MSB position of `k`), until the accumulator escapes infinity (this is the site of the historical |t|=875 branch leak). It is nonetheless not a secret-correlated leak, and the evidence is stronger than the incorrect "tiny operands don't occur" claim it replaces: the strict `scalar_multiply_ct_internal` test contrasts `k=1` (~255 infinity-iterations) against a random scalar (~0) — the worst case for infinity-escape timing — and measures **flat** (0/20). The branchless `jp_add`/`jp_double` therefore process the infinity accumulator in time indistinguishable from finite points, so the per-iteration tiny-operand latency does not accumulate into a scalar-correlated total. That makes the artefact *irrelevant to the secret scalar*, not absent. A realistic full-width-vs-full-width `fsub` test that could retire the artefact tier is tracked in [issue #78](https://github.com/sgbett/secp256k1-native/issues/78).
