...
The harness assigns each measurement to an input class **at random** — a per-measurement coin flip, as Reparaz et al. specify — not by deterministic `i & 1` alternation. This matters on real hardware: a fixed A,B,A,B cadence *aliases* with periodic ~µs transients (an SMI, a brief frequency dip on the isolated core), so in phase-aligned runs the transient lands preferentially on one class and manufactures a large, single-class |t| that is pure artefact. The [reference-machine](reference-machine.md) bring-up surfaced this — under `i & 1` the fastest field ops spiked to |t| = 218 (`fadd`) / 265 (`fred`) while the ~256× slower ladder, which averages transients out, stayed flat; randomising the class collapsed those to ~12 / ~6. Detection power is unaffected, because a genuine data-dependent leak correlates with the input **class**, not the measurement index, and so survives randomisation (the issue-#25 ladder leak still reads |t| ≈ 21). Only the aliasing artefact is removed. The figures above predate this change but are unaffected by it: that reference machine did not alias (its residual |t| was already noise-floor), so its class-assignment order made no material difference.
