...
Because the artefact magnitude is toolchain-specific, the gate's **lenient thresholds are calibrated per pinned toolchain** ([issue #74](https://github.com/sgbett/secp256k1-native/issues/74)), re-derived from the authoritative sweep whenever the compiler changes — an artefact floor, *not* loosen-to-green. The **mean** |t| is the stable, gated signal; the per-run **max** is noisy for these ~20 ns operations (across sweeps `jp_add` max ran 37/51/54 and `fsub` 27/55/26, while the means held at ~16–22 and ~11), so its bound is only a loose gross-anomaly backstop. The bounds are **tiered** so a currently-flat op cannot regress up to the widest bound unnoticed: on GCC 15.1 the elevated artefact ops (`jp_add`, `fsub`) are bounded at `mean|t|<35`, the near-flat field ops (`fadd`, `fred`, `fneg`) at a tighter `mean|t|<15`, with a shared loose `max|t|<75` backstop. These sit above the observed artefact envelope. Crucially, the lenient bounds are **not** the primary leak detector for these operations — a #25-magnitude leak (|t| ≈ 21) would in fact slip under the 35 artefact bound. That is not a gap: `jp_add`/`fsub` are point/field building blocks, not secret-scalar operations, so a secret-correlated branch in them surfaces in the **strict** `scalar_multiply_ct_internal` (gated at |t|<4.5, where the #25 ladder leak read ≈ 21 and was caught with wide margin) and in the deterministic **ctgrind** pass. Those are the security gate; the lenient bounds exist only to pin the benign diagnostic-op artefact per toolchain and flag gross regressions in the near-flat ops.
